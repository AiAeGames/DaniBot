...
class IRCConnection(object):
    ...
    nick_change_re = re.compile(
        ':(?P<old_nick>.*?)!\S+\s+?NICK\s+:\s*(?P<new_nick>[-\w]+)')
    ping_re = re.compile('^PING (?P<payload>.*)')
    chanmsg_re = re.compile(
        ':(?P<nick>.*?)\s+?PRIVMSG\s+(?P<channel>#+[-\w]+)\s+:(?P<message>[^\n\r]+)')
    privmsg_re = re.compile(
        ':(?P<nick>.*?)\s+?PRIVMSG\s+[^#][^:]+:(?P<message>[^\n\r]+)')
    part_re = re.compile(':(?P<nick>.*?)\s+?PART\s+(?P<channel>#+[-\w]+)')
    join_re = re.compile(
        ':(?P<nick>.*?)\s+?JOIN\s+.*?(?P<channel>#+[-\w]+)')
    quit_re = re.compile(':(?P<nick>.*?)\s+?QUIT\s+.*')
    ...
    def __init__(self, server, port, nick, password=None, logfile=None, verbosity=2, needs_registration=True, ssl=None):
        self.server = server
        self.port = port
    ...
    def get_logger(self, logger_name, filename):
        ...
        if self.logfile:
            handler = RotatingFileHandler(
                filename, maxBytes=1024 * 1024, backupCount=2)
    ...
    def register(self):
        ...
        self.send('USER %s %s bla :%s' %
                  (self.nick, self.server, self.nick), True)
